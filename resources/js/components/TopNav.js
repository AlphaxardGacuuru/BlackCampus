...
const TopNav = (props) => {
	...
	return (
		<div id="MyElement" style={{ display: display }} className={menu}>
					...
					<nav>
						<ul style={{ listStyle: "none" }}>
							<li className='nav-item active'>
								<Link to='/'
									style={{ color: location.pathname == "/" ? "#D0B216" : "white" }}
									className='nav-link'
									onClick={() => setMenu("")}>
							...
							<li className='nav-item active'>
								<Link to={`/profile/${props.auth.id}`}
									style={{ color: location.pathname.match("/profile") ? "#D0B216" : "white" }}
									className='nav-link'
									onClick={() => setMenu("")}>
									<span style={{ float: "left", paddingRight: "20px" }}>
			...
			{/* <!-- ***** Header Area Start ***** --> */}
			<header style={{ backgroundColor: "#182B5C" }} className="header-area">
