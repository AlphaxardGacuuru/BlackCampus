	...
	const onComment = (e) => {
		...
		axios.get('sanctum/csrf-cookie').then(() => {
			axios.post(`${props.url}/api/post-comments`, {
				...
				axios.get(`${props.url}/api/post/${id}`).then((res) => setPostComments(res.data))
			}).catch((err) => {
	...
	const onCommentLike = (comment) => {
		axios.get('sanctum/csrf-cookie').then(() => {
			axios.post(`${props.url}/api/post-comment-likes`, {
				...
				props.setMessage(res.data)
				axios.get(`${props.url}/api/post-comment-likes`).then((res) => setPostComments(res.data))
			}).catch((err) => {
				const resErrors = err.response.data.errors
				var resError
				var newError = []
	...
	const onDeleteComment = (id) => {
		axios.get('sanctum/csrf-cookie').then(() => {
			axios.delete(`${props.url}/api/post-comments/${id}`).then((res) => {
				props.setMessage(res.data)
				axios.get(`${props.url}/api/post-comments`).then((res) => setPostComments(res.data))
			}).catch((err) => {
				const resErrors = err.response.data.errors
				var resError
				var newError = []
